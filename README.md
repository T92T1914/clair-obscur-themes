...
Inter verification checks the actual rendered Regular, SemiBold and Bold faces and their genuine italic counterparts. A Windows session-availability failure was reproduced and repaired without changing the released theme files. The [acceptance record](docs/acceptance.md#installed-font-investigation) preserves the failed result, successful installed-name checks and remaining restart/native-application questions. A readable fallback is a separate result, not a passed Inter check.
