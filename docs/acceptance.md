...
| Rendered Inter faces | Session activation previously restored all six faces, but the next normal Windows restart reproduced failure of all six installed-font lookups before any repair. Fallback still passed | Resolve persistent font availability, then verify actual Equibop glyphs and preserved code/icon/language fallback |
...
The next repair candidate is Windows' supported per-user **Install** operation for the same official files, followed by fresh-process checks. It has not been executed in this record. Any immediate success must remain separate from a later normal-restart result. Do not use automatic startup activation, cache deletion or a weaker assertion to conceal the failure. Microsoft documents [temporary resource loading](https://learn.microsoft.com/en-us/windows/win32/gdi/font-installation-and-deletion) separately from its [normal font installation procedure](https://support.microsoft.com/en-us/windows/experience/personalization/manage-fonts-in-windows).

The previous session's passing results remain historical evidence. The latest required installed-font check is failed, while native typography in Equibop is unverified. Native installation, switching and restoration remain open for all four variants. The published 0.1.1 theme files are unchanged.
